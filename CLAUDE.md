...
- Route auth checks through the existing shared mechanism (`frontend/src/helpers/requireAuth.ts` on the frontend; a `requireAuth` middleware on the backend, once added) instead of re-implementing the guard inline at each call site.
...
**Audit history:** a full DRY/SOLID audit of `backend/` and `frontend/src/` was run 2026-09-07 (two independent read-only passes), producing 12 backend findings (4 high / 6 medium / 2 low) and 8 frontend findings (2 high / 4 medium / 2 low). See `todo.md`'s Phase 116 entry for the full list and verdict. Fixes are being applied incrementally, one file/commit at a time. **Re-run a pass like this periodically as the codebase grows** — this file existing is meant to stop that from being a one-time check that quietly lapses (the same failure mode issue #71's test-coverage tracking hit three separate times before this file existed).
